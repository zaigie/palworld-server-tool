<script setup>
import { AdminPanelSettingsOutlined, SupervisedUserCircleRound, SupervisedUserCircleOutlined, ContentCopyFilled } from "@vicons/material";
import { GameController, GameControllerOutline, LogOut, Ban } from "@vicons/ionicons5";
import { GatewayUserAccess } from "@vicons/carbon";
import { CrownFilled } from "@vicons/antd"
import { computed, onMounted, ref, h, defineComponent } from "vue";
import { NTag, NButton, useMessage } from "naive-ui";
import ApiService from "@/service/api";
import pageStore from "@/stores/model/page.js";
import router from "@/router";
import dayjs from "dayjs";

const avatar =
    'data:image/webp;base64,'

const pageWidth = computed(() => pageStore().getScreenWidth());
const smallScreen = computed(() => pageWidth.value < 1024);

const loading = ref(false);
const serverInfo = ref({});
const playerList = ref([]);
const playerInfo = ref({});
const skillTypeList = ref([]);

// 获取数据
const getServerInfo = async () => {
    const { data } = await new ApiService().getServerInfo()
    serverInfo.value = data.value
}
const getPlayerList = async () => {
    const { data } = await new ApiService().getPlayerList({ order_by: "last_online", desc: true })
    playerList.value = data.value
    await getPlayerInfo(playerList.value[0].player_uid)
}

const getPlayerInfo = async (player_uid) => {
    const { data } = await new ApiService().getPlayer({ playerUid: player_uid })
    playerInfo.value = data.value
    playerInfo.value.pals.forEach((pal) => {
        skillTypeList.value = skillTypeList.value.concat(pal.skills)
    })
    skillTypeList.value = [...new Set(skillTypeList.value)]
}

const isPlayerOnline = (last_online) => {
    return dayjs() - dayjs(last_online) < 60000
}
const displayLastOnline = (last_online) => {
    if (dayjs(last_online).year() < 1970) {
        return "Unknown"
    }
    return dayjs(last_online).format("YYYY-MM-DD HH:mm:ss")
}

const percentageHP = (hp, max_hp) => {
    if (max_hp === 0) {
        return 0
    }
    return ((hp / max_hp) * 100).toFixed(2)
}

const createPlayerPalsColumns = () => {
    return [{
        "title": "Type",
        "key": "type",
        // defaultSortOrder: 'ascend',
        sorter: 'default'
    }, {
        "title": "Level",
        "key": "level",
        defaultSortOrder: 'descend',
        sorter: 'default',
        render(row) {
            return "Lv." + row.level
        }
    }, {
        "title": "Melee",
        "key": "melee",
        // defaultSortOrder: 'descend',
        sorter: 'default'
    }, {
        "title": "Defense",
        "key": "defense",
        // defaultSortOrder: 'descend',
        sorter: 'default'
    }, {
        "title": "Skills",
        "key": "skills",
        render(row) {
            const skills = row.skills.map((skill) => {
                return h(
                    NTag,
                    {
                        style: {
                            marginRight: "6px"
                        },
                        type: "info",
                        bordered: false
                    },
                    {
                        default: () => skill
                    }
                );
            });
            return skills;
        },
        // defaultFilterOptionValues: ['London', 'New York'],
        filterOptions: skillTypeList.value.map(value => ({ label: value, value })),
        filter(value, row) {
            return ~row.skills.indexOf(value)
        }
    }, {
        title: "Action",
        key: "actions",
        render(row) {
            return h(
                NButton,
                {
                    size: "small",
                    onClick: () => sendMail(row)
                },
                { default: () => "Detail" }
            );
        }
    }]
}

const copyText = (text) => {
    copy(text)
}

onMounted(async () => {
    loading.value = true
    await getServerInfo()
    await getPlayerList()
    loading.value = false
})
</script>

<template>
    <div class="home-page p-3">
        <div class="bg-#fff flex justify-between items-center">
            <span class="line-clamp-1" :class="smallScreen ? 'text-lg' : 'text-2xl'">PalWorld Server Tool</span>
            <n-button type="primary" secondary strong>
                <template #icon>
                    <n-icon>
                        <AdminPanelSettingsOutlined />
                    </n-icon>
                </template>
                Auth Admin
            </n-button>
        </div>
        <div class="w-full max-w-360 mx-auto">
            <div class="rounded-lg" v-if="!loading && playerList.length > 0">
                <n-layout style="height: calc(100vh - 64px)" has-sider>
                    <n-layout-header class="p-3" style="height: 64px;" bordered>
                        <n-button-group size="large">
                            <n-button type="primary" secondary strong round>
                                <template #icon>
                                    <n-icon>
                                        <GameController />
                                    </n-icon>
                                </template>
                                Players
                            </n-button>
                            <n-button type="tertiary" secondary strong round>
                                <template #icon>
                                    <n-icon>
                                        <SupervisedUserCircleRound />
                                    </n-icon>
                                </template>
                                Guilds
                            </n-button>
                        </n-button-group>
                    </n-layout-header>
                    <n-layout position="absolute" style="top: 64px;" has-sider>
                        <n-layout-sider width="450" content-style="padding: 24px;" :native-scrollbar="false" bordered>
                            <n-list hoverable clickable>
                                <n-list-item v-for="player in playerList" :key="player.player_uid"
                                    @click="getPlayerInfo(player.player_uid)">
                                    <template #prefix>
                                        <n-avatar :src="avatar" round></n-avatar>
                                    </template>
                                    <div>
                                        <div>
                                            <n-tag :bordered="false" size="small"
                                                :type="isPlayerOnline(player.last_online) ? 'success' : 'error'" round>
                                                {{ isPlayerOnline(player.last_online) ? "Online" : "Offline" }}
                                            </n-tag>
                                            <n-tag class="ml-2" type="primary" size="small" round>
                                                Lv.{{ player.level }}
                                            </n-tag>
                                            <span class="pl-2 font-bold">{{ player.nickname }}</span>
                                        </div>
                                        <span class="inline-block mt-1 rounded-full bg-#ddd text-xs px-2 py-0.5">Last
                                            Online: {{
                                                displayLastOnline(player.last_online) }}</span>
                                    </div>
                                </n-list-item>
                            </n-list>
                        </n-layout-sider>
                        <n-layout content-style="padding: 24px;" :native-scrollbar="false">
                            <n-card :bordered="false" v-if="playerInfo.nickname">
                                <n-page-header subtitle="From the Guild of PalWorld">
                                    <n-grid :cols="6">
                                        <n-gi v-for="status in Object.entries(playerInfo.status_point)" :key="status[0]">
                                            <n-statistic :label="status[0]" :value="status[1]" />
                                        </n-gi>
                                    </n-grid>
                                    <template #title>
                                        {{ playerInfo.nickname }}
                                        <n-tag :bordered="false"
                                            :type="isPlayerOnline(playerInfo.last_online) ? 'success' : 'error'" round>{{
                                                isPlayerOnline(playerInfo.last_online) ? "Online" : "Offline" }}</n-tag>
                                    </template>
                                    <template #avatar>
                                        <n-avatar
                                            :style="{ color: 'white', backgroundColor: '#f56a00', borderRadius: '10px' }">
                                            <n-icon>
                                                <GatewayUserAccess />
                                            </n-icon>
                                        </n-avatar>
                                    </template>
                                    <template #extra>
                                        <n-space>
                                            <n-tag type="primary" size="large" round strong>
                                                Lv.{{ playerInfo.level }}
                                                <template #icon>
                                                    <n-icon :component="CrownFilled" />
                                                </template>
                                            </n-tag>
                                        </n-space>
                                    </template>
                                    <template #footer>
                                        <!-- <n-flex justify="end">Updated at 2022-01-01</n-flex> -->
                                    </template>
                                </n-page-header>
                                <n-space vertical>
                                    <n-progress type="line" status="error" indicator-placement="inside"
                                        :percentage="percentageHP(playerInfo.hp, playerInfo.max_hp)" :height="24"
                                        :border-radius="4" :fill-border-radius="0">HP: {{ percentageHP(playerInfo.hp,
                                            playerInfo.max_hp) }}%</n-progress>
                                    <n-progress type="line" indicator-placement="inside"
                                        :percentage="percentageHP(playerInfo.shield_hp, playerInfo.shield_max_hp)"
                                        :height="24" :border-radius="4" :fill-border-radius="0">SHIELD: {{
                                            percentageHP(playerInfo.shield_hp,
                                                playerInfo.shield_max_hp) }}%</n-progress>
                                </n-space>
                                <n-data-table class="mt-5" size="small" :columns="createPlayerPalsColumns()"
                                    :data="playerInfo.pals" :pagination="pagination" :bordered="false" />
                            </n-card>
                        </n-layout>
                    </n-layout>
                    <n-layout-footer class="pt-3 pr-3 bg-transparent" position="absolute" style="height: 64px;">
                        <n-flex justify="end">
                            <n-button type="error" size="large" secondary strong round>
                                <template #icon>
                                    <n-icon>
                                        <Ban />
                                    </n-icon>
                                </template>
                                Ban
                            </n-button>
                            <n-button type="warning" size="large" secondary strong round>
                                <template #icon>
                                    <n-icon>
                                        <LogOut />
                                    </n-icon>
                                </template>
                                Kick
                            </n-button>
                        </n-flex>
                    </n-layout-footer>
                </n-layout>
            </div>
            <div v-if="!loading && playerList.length === 0" class="w-full h-25 flex justify-center items-center">
                <n-empty description="什么都没有"> </n-empty>
            </div>
            <div v-if="loading" class="w-full h-25 flex justify-center items-center">
                <n-spin size="small" />
            </div>
        </div>
    </div>
</template>
